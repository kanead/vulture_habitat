# Ralph's Vulture Tracking Dataset 
...
# ralph
# Note that tracks 5013 & 5020 both white headed don't play well with this code

# select ralph data which is ralph's data from the full data set
ralph_data <- filter(mydata, study == "ralph")
ralph_data

#' Check for duplicated observations (ones with same lat, long, timestamp,
#'  and individual identifier).
ind2<-ralph_data %>% select(long, lat, id) %>%
  duplicated 
sum(ind2) 
# remove them 
ralph_data$dups <- ind2
ralph_data <- filter(ralph_data,dups=="FALSE")
ralph_data

# set the time column
levels(factor(ralph_data$id))
# can look at an individual level with 
filter(ralph_data,id=="5020")

# all of the data is in the format of day-month-year 
ralph_data$New_time<-parse_date_time(x=ralph_data$time,c("%d/%m/%Y %H:%M:%S"))

# keep only the new time data
ralph_data <- select(ralph_data, New_time,long,lat,id,species,study)
ralph_data <- rename(ralph_data, time = New_time)
ralph_data
...
library(SDLfilter)
names(ralph_data)[names(ralph_data) == 'time'] <- 'DateTime'
SDLfilterData<-ddfilter.speed(data.frame(ralph_data), vmax = 70, method = 1)
length(SDLfilterData$DateTime)

#' rename everything as before
ralph_data <- SDLfilterData
names(ralph_data)[names(ralph_data) == 'DateTime'] <- 'time'

# check the minimum time and the maximum time
min_time <- ralph_data %>% group_by(id) %>% slice(which.min(time))
data.frame(min_time)

max_time <- ralph_data %>% group_by(id) %>% slice(which.max(time))
data.frame(max_time)
...
data_summary$study <- "Ralph"
data_summary

#' can export this data summary 
#' write.csv(data_summary, file="track_resolution_summary/ralph_data_summary.csv", row.names = FALSE)
...
ralph_disp <- ggplot(trk, aes(x = t_, y=nsd_)) + geom_point()+
  facet_wrap(~id, scales="free")

ggsave("plots/ralph_disp.pdf")
ggsave("plots/ralph_disp.png")
...
#' add the study name 
timestats$study <- "ralph"
head(timestats)

#' make sure it matches up 
filter(trk,id=="5013") # wh
filter(trk,id=="5011") # wb


#' export the data 
write.csv(timestats, file="track_resolution_summary/ralph_res.csv", row.names = FALSE)
...
head(ssf.df.out)
write.csv(ssf.df.out, file="for_annotation/RalphSSFannotate.csv", row.names = FALSE)

#' we can export the whole file with all of the extra columns we added 
write.csv(ssf.df, file="amt_tracks/RalphSSFAll.csv", row.names = FALSE)
