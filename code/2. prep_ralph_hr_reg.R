...
ind2<-ralph_data %>% dplyr::select(long, lat, id) %>%
  duplicated 
sum(ind2) 
# remove them 
...
ralph_data <- dplyr::select(ralph_data, New_time,long,lat,id,species,study)
...
SDLfilterData<-ddfilter.speed(data.frame(ralph_data), vmax = 100, method = 1)
...
write.csv(x = trk4, file = "regularised/ralph_reg.csv", row.names = FALSE)
