...
ralph_data$New_time<-parse_date_time(x=ralph_data$time,c("%d/%m/%Y %H:%M"))
...
SDLfilterData<-ddfilter.speed(data.frame(ralph_data), vmax = 60, method = 1)
...
# try the amt package 
trk <- mk_track(ralph_data, .x=long, .y=lat, .t=time, id = id, species=species, 
