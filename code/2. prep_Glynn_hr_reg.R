...
ind2 <- Glynn_data %>% dplyr::select(long, lat, id) %>%
  duplicated
sum(ind2)
# remove them
...
Glynn_data <-
  dplyr::select(Glynn_data, New_time, long, lat, id, species, study)
...
SDLfilterData <-
  ddfilter.speed(data.frame(Glynn_data), vmax = 100, method = 1)
