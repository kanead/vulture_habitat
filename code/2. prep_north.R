# Darcy's North Vulture Tracking Dataset 
...
# select north data which is Darcy's data 
north_data <- filter(mydata, study == "north")
north_data

#' Check for duplicated observations (ones with same lat, long, timestamp,
#'  and individual identifier).
ind2<-north_data %>% select(long, lat, id) %>%
  duplicated 
sum(ind2) 
# remove them 
north_data$dups <- ind2
north_data <- filter(north_data,dups=="FALSE")
north_data

# set the time column
levels(factor(north_data$id))
# can look at an individual level with 
(filter(north_data,id=="Bastard"))

# all of the data is in the format of day-month-year 
north_data$New_time<-parse_date_time(x=north_data$time,c("%d/%m/%Y %H:%M"))

# keep only the new time data
north_data <- select(north_data, New_time,long,lat,id,species,study)
north_data <- rename(north_data, time = New_time)
north_data

# check the minimum time and the maximum time
min_time <- north_data %>% group_by(id) %>% slice(which.min(time))
data.frame(min_time)


max_time <- north_data %>% group_by(id) %>% slice(which.max(time))
data.frame(max_time)

#' filter extreme data based on a speed threshold 
#' based on vmax which is km/hr
#' time needs to be labelled DateTime for these functions to work
names(north_data)[names(north_data) == 'time'] <- 'DateTime'
SDLfilterData<-ddfilter.speed(data.frame(north_data), vmax = 60, method = 1)
length(SDLfilterData$DateTime)

#' rename everything as before
north_data <- SDLfilterData
names(north_data)[names(north_data) == 'DateTime'] <- 'time'

# try the amt package 
trk <- mk_track(north_data, .x=long, .y=lat, .t=time, id = id, species=species,
...
north_disp <- ggplot(trk, aes(x = t_, y=nsd_)) + geom_point()+
  facet_wrap(~id, scales="free")

ggsave("plots/north_disp.pdf")
ggsave("plots/north_disp.png")
...
#' add the study name 
timestats$study <- "north"
head(timestats)

#' make sure it matches up 
filter(trk,id=="Spot") # wb
filter(trk,id=="Simons") # rv


#' export the data 
write.csv(timestats, file="track_resolution_summary/north_res.csv", row.names = FALSE)
...
#' select individuals that have temporal resolution of ~ 1 hour
trk<- filter(trk,id=="Tapika"|
               id=="Bastard"|
               id=="ThreeInOne"|
               id=="Santa")
...
head(ssf.df.out)
write.csv(ssf.df.out, file="amt_tracks/NorthSSFannotate.csv", row.names = FALSE)

#' we can export the whole file with all of the extra columns we added 
write.csv(ssf.df, file="amt_tracks/NorthSSFAll.csv", row.names = FALSE)
