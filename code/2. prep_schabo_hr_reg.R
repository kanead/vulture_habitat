...
ind2 <- Schabo_data %>% dplyr::select(long, lat, id) %>%
  duplicated
sum(ind2)
# remove them
...
Schabo_data <-
  dplyr::select(Schabo_data, New_time, long, lat, id, species, study)
...
SDLfilterData <-
  ddfilter.speed(data.frame(Schabo_data), vmax = 100, method = 1)
...
write.csv(x = trk4, file = "regularised/schabo_reg.csv", row.names = FALSE)
