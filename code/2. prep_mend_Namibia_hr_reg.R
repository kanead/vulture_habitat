...
ind2<-mend_data %>% dplyr::select(long, lat, id) %>%
  duplicated 
sum(ind2) 
# remove them 
mend_data$dups <- ind2
...
mend_data <- dplyr::select(mend_data, New_time,long,lat,id,species,study)
...
SDLfilterData<-ddfilter.speed(data.frame(mend_data), vmax = 100, method = 1)
