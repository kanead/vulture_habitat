...
ind2<-masai_data %>% dplyr::select(long, lat, id) %>%
  duplicated 
sum(ind2) 
# remove them 
...
SDLfilterData<-ddfilter.speed(data.frame(masai_data), vmax = 100, method = 1)
...
write.csv(x = trk4, file = "regularised/masai_reg.csv", row.names = FALSE)
