...
ind2<-inter_data %>% dplyr::select(long, lat, id) %>%
  duplicated 
sum(ind2) 
# remove them 
...
inter_data <- dplyr::select(inter_data, New_time,long,lat,id,species,study)
...
SDLfilterData<-ddfilter.speed(data.frame(inter_data), vmax = 100, method = 1)
...
write.csv(x = trk4, file = "regularised/inter_reg.csv", row.names = FALSE)
