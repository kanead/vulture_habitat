...
ind2<-north_data %>% dplyr::select(long, lat, id) %>%
  duplicated 
sum(ind2) 
# remove them 
...
SDLfilterData<-ddfilter.speed(data.frame(north_data), vmax = 100, method = 1)
...
write.csv(x = trk4, file = "regularised/north_reg.csv", row.names = FALSE)
...
data_summary$study <- "north"
data_summary

#' can export this data summary 
write.csv(data_summary, file="track_resolution_summary/north_data_summary.csv", row.names = FALSE)
